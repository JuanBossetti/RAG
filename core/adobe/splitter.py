...
def process_zip(input_zip):
    ...
    _concatenate_chunks(chunks, input_zip.replace('.zip', '.txt'))
    return chunks
...
def _concatenate_chunks(chunks, file_name):
    concatenated_content = ""

    for i, chunk in enumerate(chunks):
        concatenated_content += f"--------- Chunk{i+1} ---------\n\n"
        concatenated_content += chunk.page_content + "\n\n"
